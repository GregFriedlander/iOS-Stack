//: Playground - noun: a place where people can play

import UIKit

var str = "Hello, playground"

class Animal {
    var name: String
    var health = 100
    init(name: String) {
        self.name = name
    }
    func displayHealth() {
        print("My health is at \(self.health)")
    }
}

class Cat: Animal {
    init(catName: String) {
        super.init(name: catName)
        self.health = 150
    }
    func growl() {
        print("Rawr!")
    }
    func run() -> Cat {
        print("Running")
        health -= 10
        return self
    }
}

class Lion: Cat {
    init(lionName: String){
        super.init(catName: lionName)
        self.health = 200
    }
    override func growl() {
        print("ROAR!!!! I am the King of the Jungle")
    }
}

class Cheetah: Cat {
    init(cheetahName: String){
        super.init(catName: cheetahName)
    }
    override func run() -> Cheetah{
        if self.health >= 50 {
            print("Running")
            health -= 50
        }
        return self
    }
    func sleep () {
        if self.health + 50 <= 200 {
            self.health += 50
        }
    }
}

let Simba = Cheetah(cheetahName: "Simba")
Simba.run().run().run().run().displayHealth()

let Tony = Lion(lionName: "Tony")
Tony.run().run().run().growl()